...
class ReplayBuffer(object):
    def __init__(self, batch_size, state_history_size=4):
        ...
        self.garbage_check = 1.0
        self.state_history_size = state_history_size
        self.state_info = None
        self.random_state = np.random.RandomState(100)
    ...
    def add_experience(self, partial_experience):
        ...
        # a tuple with state in the 0th entry
        # all other info afterwards
        # compress and rebuild experience since we don't want copies
        self.state_info = {}
        self.state_info["shape"] = experience[0].shape
        self.state_info["dtype"] = experience[0].dtype

        S_prime_bytes = experience[0].tostring()
        z_S_prime = zlib.compress(S_prime_bytes)
        # all of this is to minimize memory
        ...
        # + 1 extra to account for the offset of S / S_prime in the lookup
        # garbage check so that we don't do this garbage collection too often
        if len(self.memory) > (self.garbage_check * self.max_buffer_size + 1):
    ...
    def get_minibatch(self, index_list=None):
        if len(self.memory) > self.batch_size + self.state_history_size + 1:
            ...
            for r_e in return_experience:
                ...
                # decompress
                b_i = (np.stack(_all_S[:-1]).transpose(1, 2, 0)[None], np.stack(_all_S[1:]).transpose(1, 2, 0)[None],) + r_e[1:]
                b.append(b_i)
            # make a proper minibatch here?
            # current format is list of tuple
            S = np.concatenate([bi[0] for bi in b])
            S_prime = np.concatenate([bi[1] for bi in b])
            stacked_tuple_info = [bi[2:] for bi in b]
            return S, S_prime, stacked_tuple_info
        else:
           return None
